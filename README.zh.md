...
- **文件搜索** — 支持通配符和正则表达式搜索
- **文件提取** — 支持保留目录结构批量导出
- **高级图片查看器** — 支持 BLP（War3/WoW）纹理的 MIP 贴图切换和动画播放
...
- **图片格式**：支持 BLP2 未压缩纹理。DXTC 压缩的 BLP 和 DDS 纹理（SC2 常用）需要额外的解码库。

## 注意事项

- **只读工具**：本工具不会以任何方式修改 CASC 存储
- **在线 CDN**：按需下载和缓存游戏数据分块
- **图片查看器**：支持 BLP2 未压缩纹理（War3/WoW）。DXTC 压缩的 BLP 和 DDS 纹理（SC2）需要额外的解码器库
